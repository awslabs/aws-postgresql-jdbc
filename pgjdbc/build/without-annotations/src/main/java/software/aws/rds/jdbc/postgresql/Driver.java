...
public class Driver extends org.postgresql.Driver {
  ...
  private static boolean awsProtocolOnly = false;
  ...
  public static void setAwsProtocolOnly(boolean onlyAwsProtocol) {
    awsProtocolOnly = onlyAwsProtocol;
  }
  ...
  public static /* @Nullable */ Properties parseURL(String url, /* @Nullable */ Properties defaults) {
    ...
    final String acceptedPrefix = awsProtocolOnly ? AWS_PROTOCOL : POSTGRES_PROTOCOL;

    if (!urlServer.startsWith(acceptedPrefix)) {
      final String errMsgModifier = awsProtocolOnly ? "" : " \"" + POSTGRES_PROTOCOL + "\" or ";

      final String errorMessage = "JDBC URL must start with " + errMsgModifier
