...
public class AuroraTopologyService implements TopologyService {
  ...
  public AuroraTopologyService(int refreshRateInMilliseconds) {
    ...
    this.clusterInstanceTemplate = new HostInfo(null, null, false, HostInfo.initProps("?", HostInfo.NO_PORT, null));
  }
  ...
  public void setClusterInstanceTemplate(HostInfo clusterInstanceTemplate) {
    LOGGER.log(Level.FINER, "[AuroraTopologyService] clusterInstance host=''{0}'', port={1,number,#}, database=''{2}''",
   ...
   */
  @Override
  public /* @Nullable */ List</* @Nullable */ HostInfo> getTopology(Connection conn, boolean forceUpdate) {
  ...
  protected ClusterTopologyInfo queryForTopology(Connection conn) {
    ...
    List</* @Nullable */ HostInfo> hosts = new ArrayList<>();
    int writerCount = 0;
    try (Statement stmt = conn.createStatement()) {
      try (ResultSet resultSet = stmt.executeQuery(RETRIEVE_TOPOLOGY_SQL)) {
        hosts.add(null); // reserve space for a writer node
  ...
  private HostInfo createHost(ResultSet resultSet, boolean isWriter) throws SQLException {
    ...
    String hostEndpoint = getHostEndpoint(hostName);
    HostInfo hostInfo = new HostInfo(
        getUrlFromEndpoint(hostEndpoint, this.clusterInstanceTemplate.getPort(), this.clusterInstanceTemplate.getDatabase()),
        hostName,
        isWriter,
        this.clusterInstanceTemplate.copyProps());
    hostInfo.updateHostProp(hostEndpoint);
    return hostInfo;
   ...
   */
  @Override
  public /* @Nullable */ List</* @Nullable */ HostInfo> getCachedTopology() {
  ...
  private static class ClusterTopologyInfo {
    public List</* @Nullable */ HostInfo> hosts;
     ...
     */
    ClusterTopologyInfo(List</* @Nullable */ HostInfo> hosts, Set<String> downHosts,
